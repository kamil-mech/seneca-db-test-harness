...
if (info.launchType === 'db') {
  setTerminalTitle(info.db.label);
  ...
  setTerminalTitle(info.image.label);
  ...
  var cp = spawn('bash', ['-c', 'cd ' + info.testpath + '; npm test --db=' + info.db.name + '-store --ip=' + info.imagecontainer.ip + ' --port=' + info.imagecontainer.port]);
}
