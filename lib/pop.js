...
if (info.launch === 'db') {
  ...
  if (info.db === 'mysql') {
    // mysql
    // enchancement: write utility for spawning and exec'ing
    calls.push(function(next){
      var cp = spawn('bash', [__dirname + '/run-mysql.sh', info.dbOptions.schema, info.dbOptions.user, info.dbOptions.password,
                               info.dbOptions.name, __dirname + '/../temp/' + info.dblabel + '.cid']);
      next();
    });
  } else {
    // regular
  ...
  var cp = spawn('bash', ['-c', 'cd ' + info.app.path + ' && npm test --db=' + info.db + '-store --ip=' + info.imgcontainer.ip + ' --port=' + info.imgcontainer.port]);
}
