...
function justWarning (data) {
  var knownWarnings = []
  if (info.db && info.db.dbconst.knownWarnings) {
    _.each(info.db.dbconst.knownWarnings, function (warning) {
      knownWarnings.push(warning)
    })
  }

  if (info.app && info.app.conf.knownWarnings) {
    _.each(info.app.conf.knownWarnings, function (warning) {
      knownWarnings.push(warning)
    })
  }
  ...
  _.each(knownWarnings, function (warning) {
    if (!justWarning && lowdata.indexOf(warning.toLowerCase()) > -1) justWarning = true
  })
  return justWarning
}
